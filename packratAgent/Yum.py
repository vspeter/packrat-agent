...
class YUMManager(LocalRepoManager):
    ...
    def addEntry(self, type, filename, distro, distro_version, arch):
        ...
        logging.debug(('yum: Got Entry for package: %s arch: %s distro: %s ' +
                      'distro_version: %s') %
                      (filename, arch, distro, distro_version))
        ...
        full_rpm_path = '%s/%s/%s/%s/%s/%s' % (self.root_dir, distro,
                                               self.component, distro_version,
                                               arch, filename)
        ...
        for arch in arch_list:
            self.entry_list[distro][distro_version][arch][filename] = (
                sha1, size, changelog, fields)

    def loadFile(self, file_name, temp_file, distro, distro_version, arch):
        dir_path = '%s/%s/%s/%s/%s/' % (self.root_dir, distro,
                                        self.component, distro_version,
                                        arch)
    ...
    def checkFile(self, file_name, distro, distro_version, arch):
        rpm_path = '%s/%s/%s/%s/%s/%s' % (self.root_dir, distro,
                                          self.component, distro_version,
                                          arch, file_name)
    ...
    def _writeArchMetadata(self, base_path, distro, distro_version, arch):
        ...
        wrk.write('<otherdata xmlns="http://linux.duke.edu/metadata/other" ' +
                  'packages="%s">\n' %
                  len(self.entry_list[distro][distro_version][arch]))
        for filename in self.entry_list[distro][distro_version][arch]:
            (sha1, size, changelog,
             fields) = self.entry_list[distro][distro_version][arch][filename]
            ...
            for change in changelog:
                wrk.write(('    <changelog author="%s; %s-%s" date="%s">%s' +
                           '</changelog>\n')) % (change['auth'],
                                                 change['version'],
                                                 change['rev'].split('.')[0],
                                                 change['date'],
        ...
        repo_files.append({'type': 'other', 'href': 'other.xml',
                           'checksum': sha1, 'open-checksum': sha1orig})
        ...
        repo_files.append({'type': 'filelists', 'href': 'filelists.xml',
                           'checksum': sha1, 'open-checksum': sha1orig})

        full_path = '%s/primary.xml' % dir_path
        wrk = open(full_path, 'w')
        wrk.write('<?xml version="1.0" encoding="UTF-8"?>\n')
        wrk.write(('<metadata packages="%s" xmlns="http://linux.duke.edu/' +
                   'metadata/common" xmlns:rpm="http://linux.duke.edu/met' +
                   'adata/rpm">\n') %
                  len(self.entry_list[distro][distro_version][arch]))
        ...
        (sha1orig, sha256orig, md5orig) = hashFile(full_path)
        (sha1, sha256, md5) = hashFile(full_path)
        repo_files.append({'type': 'primary', 'href': 'primary.xml',
                           'checksum': sha1, 'open-checksum': sha1orig})
